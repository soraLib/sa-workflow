...
const createPredicate = <T extends TreeNode>(
  arg1: keyof T | PredicateCallback<T>,
  arg2?: T[keyof T]
) => {
...
const findNodeHelper = <T extends TreeNode>(
  node: T,
...
const removeNodeHelper = <T extends TreeNode>(
  node: T,
  predicate: PredicateCallback<T>,
  tree: T[],
  index: number
  ...
  if (node.children?.length) {
    for (let i = 0; i < node.children.length; i++) {
      const removeChildNode = removeNodeHelper(
        <T>node.children[i],
        predicate,
        <T[]>node.children,
        i
